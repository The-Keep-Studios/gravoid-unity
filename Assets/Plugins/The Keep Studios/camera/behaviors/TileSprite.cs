...
namespace TheKeepStudios{
	public class TileSprite : MonoBehaviour{
		...
		void OnDrawGizmosSelected(){
			...
			Vector3 cubesize = new Vector3(tileSize, tileSize, tileSize);
			int numTiles = this.tiles.Count > 0 ? this.tiles.Count : (gridSize.x * gridSize.y);
			for(int tiles_idx = 0; tiles_idx < numTiles; ++tiles_idx){
				Vector3 tilePos = getTilePosition(tiles_idx) + myPosition;
				Gizmos.DrawWireCube(tilePos, cubesize);
				UnityEditor.Handles.Label(tilePos, string.Format("{0} : #{1,-5:D}", transform.gameObject.name, tiles_idx));
			}
		}
