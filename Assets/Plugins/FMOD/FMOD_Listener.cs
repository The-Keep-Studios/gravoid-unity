...
public class FMOD_Listener : MonoBehaviour 
{
	...
	void LoadPlugins()
	{
		...
		foreach (var name in pluginPaths)
		{
			...
			bool pluginExists = System.IO.File.Exists(path);

			if (path.EndsWith(".bundle"))
			{
				pluginExists = System.IO.Directory.Exists(path);
			}
			...
			uint handle;
			FMOD.RESULT result = sys.loadPlugin(path, out handle);

			if (result != FMOD.RESULT.OK)
			{
				FMOD.Studio.UnityUtil.LogError("Unable to load plugin: " + name);
			}
