...
namespace TheKeepStudios.Gravoid.CUBS{
	...
	#region PUBLIC METHODS
		...
		public ProjectileBehavior GetProjectile(ICUBSConfiguration configuration, ProjectileBehavior projectilePrefab){

			List<PartSelectionBehavior> selection = configuration.getPartSelectionList();
		...
		private List<PartSelectionBehavior> GetSelections(List<PartSelectionBehavior> _selection){
			...
			if(lackingComponents.Count == 0){
				...
				foreach(PartSelectionBehavior nextselectionType in _selection){
					...
					//FIXME This needs to actually have a better determination of where the selection needs to go
